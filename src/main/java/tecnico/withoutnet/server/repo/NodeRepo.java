...
public interface NodeRepo extends JpaRepository<Node, Integer> {
    ...
    @Query("SELECT nd FROM Network nw INNER JOIN Node nd ON nw.id = :networkId")
    List<Node> findNodeByNetworkId(@Param("networkId") int networkId);

    @Query("SELECT nd FROM Network nw INNER JOIN Node nd ON nw.id = :networkId AND nd.commonName LIKE :commonNamePattern")
